...
namespace DynamicCore.HelpPageLibrary
{
    public class XmlDocumentationProvider : IDocumentationProvider
    {
        ...
        public XmlDocumentationProvider( string documentPath )
        {
            ...
            if( documentPath.Contains( "," ) == true )
            {
                foreach( var path in documentPath.Split( ',' ) )
                {
                    foreach( string file in Directory.GetFiles( path , string.Format( "*{0}.xml" , DynamicCoreConstants.DllSuffix ) ) )
                    {
            ...
            {
                foreach( string file in Directory.GetFiles( documentPath , string.Format( "*{0}.xml" , DynamicCoreConstants.DllSuffix ) ) )
                {
                    using( var fileStream = File.OpenRead( file ) )
                    {
                        if( finalDoc == null )
